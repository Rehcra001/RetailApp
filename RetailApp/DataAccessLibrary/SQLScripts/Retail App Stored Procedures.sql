...
--Month to date
--Revenue MTD
--Returns the total revenue by invoice for the month
--or an error message on failure
CREATE PROCEDURE dbo.usp_RevenueMTD AS
BEGIN
	BEGIN TRY
		SELECT ISNULL(SUM(I.QuantityIssued * SOD.UnitPrice * -1),0) AS RevenueMTD
		FROM dbo.Issues AS I
		JOIN dbo.SalesOrderDetail AS SOD ON I.SalesOrderID = SOD.SalesOrderID
		WHERE YEAR(IssueDate) = YEAR(GETDATE()) AND 
			  MONTH(IssueDate) = MONTH(GETDATE()) AND
			  I.ReverseReferenceID IS NULL;
	END TRY
...
--Products by revenue MTD
--Returns all invoiced products group by revenue
--or an error message on failure
CREATE PROCEDURE dbo.usp_ProductsByRevenueMTD AS
BEGIN
	BEGIN TRY
		--Products by revenue MTD
		;WITH SalesMTD AS
		(
			SELECT I.ProductID, PR.ProductName, I.QuantityIssued * -1 AS QuantityInvoiced, SOD.UnitPrice
			FROM dbo.Issues AS I
			JOIN dbo.SalesOrderDetail AS SOD ON I.SalesOrderID = SOD.SalesOrderID
			JOIN dbo.Products AS PR ON SOD.ProductID = PR.ProductID
			WHERE YEAR(IssueDate) = YEAR(GETDATE()) AND 
				  MONTH(IssueDate) = MONTH(GETDATE()) AND
				  I.ReverseReferenceID IS NULL
		),
		RevenueByProduct AS
		(
			SELECT ProductID, ProductName, ISNULL(SUM(QuantityInvoiced * UnitPrice),0) AS RevenueByProductMTD
			FROM SalesMTD
			GROUP BY ProductID, ProductName
		)
		SELECT ProductName, RevenueByProductMTD
		FROM RevenueByProduct
		ORDER BY RevenueByProductMTD DESC;
	END TRY
...
--Revenue by top 10 products MTD
--returns the revenue generated by the top 10 products for this month
--else returns an error message on failure
CREATE PROCEDURE dbo.usp_GetRevenueTop10ProductsMTD AS
BEGIN
	BEGIN TRY
		;WITH SalesMTD AS
		(
		...
		SELECT ISNULL(SUM(QuantityInvoiced * UnitPrice),0) AS RevenueByProductMTD
		FROM SalesMTD;
	END TRY
...
--Returns the top 10 products by revenue for MTD
--or an error message on failure
CREATE PROCEDURE dbo.usp_Top10ProductsByRevenueMTD AS
BEGIN
	BEGIN TRY
		...
		SELECT TOP(10) ProductName, RevenueByProductMTD
		FROM RevenueByProduct
		ORDER BY RevenueByProductMTD DESC;
	END TRY
...
--Number of sales order created this month
--Returns the number of sales orders created this month
--else returns an error message on failure
CREATE PROCEDURE dbo.usp_CountOfSalesOrdersMTD AS 
BEGIN
	BEGIN TRY
		--Count of sales order created this month
		SELECT CAST(COUNT(*) AS DECIMAL) AS CountOfSalesOrdersMTD
		FROM dbo.SalesOrderHeader AS SOH
		WHERE YEAR(SOH.OrderDate) = YEAR(GETDATE()) AND
			  MONTH(SOH.OrderDate) = MONTH(GETDATE());
	END TRY
...
--Number of open sales orders created this month
--Returns the number of sales orders still open created this month
--else returns an error message on failure
CREATE PROCEDURE dbo.usp_GetCountOfOpenSalesOrdersMTD AS
BEGIN
	BEGIN TRY
		--Count of open sales orders created this month
		SELECT CAST(COUNT(*) AS DECIMAL) AS CountOfOpenSalesOrdersMTD
		FROM dbo.SalesOrderHeader AS SOH
		WHERE SOH.OrderStatusID = 1 AND
			  YEAR(SOH.OrderDate) = YEAR(GETDATE()) AND
			  MONTH(SOH.OrderDate) = MONTH(GETDATE());
	END TRY
...
--Number of cancelled sales orders
--Returns the numbers of sales orders cancelled that were created this month
--else returns an error message on failure
CREATE PROCEDURE dbo.usp_GetCountOfCancelledSalesOrdersMTD AS
BEGIN
	BEGIN TRY
		--Count of open sales orders created this month
		SELECT CAST(COUNT(*) AS DECIMAL) AS CountOfCancelledSalesOrdersMTD
		FROM dbo.SalesOrderHeader AS SOH
		WHERE SOH.OrderStatusID = 4 AND
			  YEAR(SOH.OrderDate) = YEAR(GETDATE()) AND
			  MONTH(SOH.OrderDate) = MONTH(GETDATE());
	END TRY
...
--Gets the count of days to close a sales order created this month
--Returns a count of days to close sales order
--else returns an error message on failure
CREATE PROCEDURE dbo.usp_GetDaysCountToCloseSalesOrderMTD AS
BEGIN
	BEGIN TRY
		;WITH SalesOrdersClosedMTD AS
		(
			SELECT SOH.SalesOrderID, SOH.OrderDate
			FROM dbo.SalesOrderHeader AS SOH
			WHERE (SOH.OrderStatusID = 2 OR
				  SOH.OrderStatusID = 3) AND
				  YEAR(SOH.OrderDate) = YEAR(GETDATE()) AND
				  MONTH(SOH.OrderDate) = MONTH(GETDATE())
		),
		MaxDateClosedMTD AS
		(
			SELECT I.SalesOrderID, MAX(I.IssueDate) AS DateClosed
			FROM Issues AS I
			WHERE I.ReverseReferenceID IS NULL AND
				  YEAR(I.IssueDate) = YEAR(GETDATE()) AND
